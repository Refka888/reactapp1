...
export default function Home() {
  ...
  const fetchData = async (e) => {
    const result1 = await axios.get(
      "https://api.chucknorris.io/jokes/search?query=lower"
    );
    const result2 = await axios.get(
      "https://api.chucknorris.io/jokes/search?query=doctor"
    );
    const result3 = await axios.get(
      "https://api.chucknorris.io/jokes/search?query=business"
    );
    const result4 = await axios.get(
      "https://api.chucknorris.io/jokes/search?query=police"
    );
    const result5 = await axios.get(
      "https://api.chucknorris.io/jokes/search?query=boss"
    );
    setState({
      ...state,
  ...
  return (
    <div>
      <section className="container">
        ...
        <article>
          <p className="view">
            VIEW MORE <img src={img8} alt="fleche" />
          </p>
        </article>
      </section>
    </div>
